...
def need(name):
    ...
    if not value:
        print(f"\n{'!' * 60}\n  Secret '{name}' 이(가) 비어 있습니다.\n"
              f"  Settings → Secrets and variables → Actions 에서 확인하세요.\n"
              f"{'!' * 60}\n", flush=True)
        sys.exit(1)
    return value
...
def refresh_kakao_token():
    ...
    if res.status_code != 200:
        raise RuntimeError(
            ...
            "→ get_token.ps1을 다시 실행해서 새 토큰을 Secret에 넣어주세요."
        )
    data = res.json()
...
def find_free_image(soup):
    ...
    for img in soup.find_all("img"):
        ...
        if caption and any(h in caption.lower() for h in PUBLIC_DOMAIN_HINTS):
            log(f"  퍼블릭 도메인 사진 발견: {caption[:60]}")
...
def fetch_article(url):
    ...
    return {"title": title, "body": body[:6000], "url": url,
            "video": find_video(res.text), "image": find_free_image(soup)}
...
<style>
  *{{box-sizing:border-box}}
  ...
  .w:active,.w.on{{background:#ffe9a8}}
...
function speak(text, rate, onEnd) {{
  if (!window.speechSynthesis) {{
    alert('이 브라우저는 읽어주기를 지원하지 않아요. 크롬으로 열어보세요.');
    return;
  }}
...
function sayWord(el) {{ speak(el.textContent.trim(), 0.75); }}
...
def render_media(art):
    ...
    img = art.get("image")
    if img:
        parts.append(f'<img class="hero" src="{esc(img["src"])}" alt="">'
                     f'<p class="hero-cap">{esc(img["caption"])}</p>')
    vid = art.get("video")
    if vid:
        parts.append(f'<a class="vid-btn" href="https://www.youtube.com/watch?v={esc(vid)}" '
...
def render_quiz(quiz):
    ...
    for i, q in enumerate(quiz, 1):
        ...
        blocks.append(f'  <div class="qz" data-a="{ans}">\n'
...
def render_quiz_answers(quiz):
    ...
    for i, q in enumerate(quiz, 1):
        ...
        rows.append(f"<tr><td>{i}번</td>"
                    f"<td><b>{chr(65 + ans)}. {esc(correct)}</b><br>"
                    f"<span style='color:#777;font-size:14px'>"
                    f"{esc(q.get('why_ko',''))}</span></td></tr>")
    return "<table><tr><th>문제</th><th>정답과 해설</th></tr>" + "".join(rows) + "</table>"
...
def render_html(c, source_url, art=None):
    ...
    article_html = "\n".join(f"  <p>{wrap_words(p, glossary)}</p>"
                             for p in c["article_en"])
    ...
    if lv:
        ...
        level_note = (
            ...
            "르네상스러닝의 <b>공식 AR(ATOS) 지수는 아닙니다.</b> "
...
def main():
    ...
    history.setdefault("issues", []).append({
        ...
        "title_en": content["title_en"], "title_ko": content["title_ko"]})
